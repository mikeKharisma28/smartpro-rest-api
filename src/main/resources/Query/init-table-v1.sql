...
create table MasterData.Menu (Id bigint identity not null, CreatedBy bigint not null, CreatedDate datetime2(6) not null, Name varchar(70), UpdatedBy bigint, UpdatedDate datetime2(6), Url varchar(255), ParentId bigint, primary key (Id));
go
...
create table MasterData.AccessPermission (Id bigint identity not null, AllowCreate bit not null, AllowDelete bit not null, AllowRead bit not null, AllowUpdate bit not null, CreatedBy bigint not null, CreatedDate datetime2(6) not null, UpdatedBy bigint, UpdatedDate datetime2(6), DivisionId bigint not null, MenuId bigint not null, primary key (Id));
go

create unique nonclustered index UK7ko57exlxsklwniiwlieg5fn8 on MasterData.Division (Name) where Name is not null;
go

create unique nonclustered index UKtgpon4wg3huqbrk29wujgveox on MasterData.Menu (Name) where Name is not null;
go
...
alter table MasterData.AccessPermission add constraint FKagj61ky2sbon3bkkdyaiyy2c7 foreign key (DivisionId) references MasterData.Division;
go
